...
def branding_patch():
	...
	update_field_label()
	if cint(get_frappe_version()) >= 13 and not frappe.db.get_single_value('branding Setting', 'ignore_onboard_branding'):
...
def boot_session(bootinfo):
	...
	if frappe.session['user']!='Guest':
		...
		bootinfo.branding_setting = frappe.get_doc("branding Setting","branding Setting")

@frappe.whitelist()
def ignore_update_popup():
	if not frappe.db.get_single_value('branding Setting', 'disable_new_update_popup'):
